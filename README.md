...
8. **The white rabbit** — every so often, a small, compact always-white pixel rabbit (side-on, ears back, one eye, a little cottontail) appears at the bottom of the screen, holds still for a beat, then leaps left to right in a handful of big, choppy hops rather than a smooth run — it snaps to each new spot the instant the frame changes and holds still through the rest of that hop's pose, the way an old 8-bit sprite actually moved (no sliding, no rotation, no squash-and-stretch). Sometimes, instead of crossing the bottom, it sneaks up to a hole in one of the title letters (the O's, A's, B's, D's, P's, Q's, and R's) and hides inside it for a moment before dashing back off the bottom of the screen. Click it before it escapes: the first catch turns the whole site red, the second turns it pink, and every catch after that shifts it to a new random neon color from a much wider palette (24 hues) — and it explodes into a shower of 0s and 1s that also ripples outward through the matrix rain behind it, no popup needed. From the third catch on, the new color sometimes arrives as a flat tone like before, but can also land as a two-tone gradient across the progress bars and rank badge, a faint diagonal stripe pattern over the whole page, or a scattered dot-grid texture — so the same catch doesn't always look the same way twice. Whatever color and style you land on sticks around (saved per browser) until you catch it again.
